...
namespace BlazorMenu.Shared.MenuOverlay
{
    ...
    {
        internal event Func<DrawerMenuItem, string[]?, Task>? OnShow;
        internal event Func<Task>? OnHide;
        internal event Action<Func<string, string, Task>>? OnAssignOnClick;

        public async Task Show(DrawerMenuItem poMenu, string[]? poBreadCrumbs = null) => await (OnShow?.Invoke(poMenu, poBreadCrumbs) ?? Task.CompletedTask);
